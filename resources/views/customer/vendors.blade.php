...
<div class="bg-white min-h-screen">
        ...
        <div class="max-w-7xl mx-auto px-6 py-4">
            <div class="flex flex-col md:flex-row gap-4 items-center justify-between">
                ...
                <div class="flex gap-3 overflow-x-auto w-full md:w-auto pb-2 md:pb-0">
                    <select id="filterKategori" class="px-4 py-2 bg-gray-50 border border-gray-200 rounded-full text-sm font-medium text-gray-700 focus:outline-none cursor-pointer">
                    ...
                    <select id="filterHarga" class="px-4 py-2 bg-gray-50 border border-gray-200 rounded-full text-sm font-medium text-gray-700 focus:outline-none cursor-pointer">
    ...
    <main class="max-w-7xl mx-auto px-6 pb-20">
        ...
        @if($allPackages->isNotEmpty())
            <div id="katalogGrid" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                
                @foreach($allPackages as $pkg)
                <a href="{{ route('packages.show', ['id' => $pkg->id]) }}" class="katalog-item group cursor-pointer block"
                   ...
                   data-price="{{ (float)$pkg->price }}">
                    ...
                    <div class="relative aspect-[4/5] rounded-3xl overflow-hidden mb-5 bg-gray-100 shadow-sm border border-gray-50">
                        <img src="{{ $pkg->main_image ?: 'https://picsum.photos/600/800' }}" 
                    ...
                    <div class="flex justify-between items-start px-1">
                        <div class="pr-4">
                            ...
                            <div class="flex items-center text-gray-500 text-sm">
                                <span>🌿 By {{ $pkg->business_name ?? 'NaturaWed Vendor' }}</span>
                            </div>
                        </div>
